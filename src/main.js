...
const findItem = (keyword) => {
	return itemList.find(
		(item) =>
			item.itemNumber === parseInt(keyword) ||
...
const announceWinner = (finalData) => {
	finalData.map((item) => {
		if (item && item.currentBuyer && item.currentBid > item.reservePrice) {
			...
			console.log(
				`${item.description} is sold to buyer ${item?.currentBuyer} with $${totalItemFee} \n`
			);
		}
	});
};
...
const callHomeInterface = () => {
	switch (
		...
		case "S":
			...
			console.log(
				"Current Highest Bid is =>" + foundItem.currentBid ||
					foundItem.initialPrice
			);
